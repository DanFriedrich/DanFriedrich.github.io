...
class Table{
    ...
    createFelt(){
        ...
        const floorBox = new CANNON.Box(new CANNON.Vec3(mainAreaX,
        floorThickness, Table.WIDTH/2));
        const floorBoxSmall = new CANNON.Box(new CANNON.Vec3
        (narrowStripWidth, floorThickness, narrowStripLength));
        ...
        body.addShape(floorBoxSmall, new CANNON.Vec3(-mainAreaX - 
        narrowStripWidth, -floorThickness, 0));
        body.addShape(floorBoxSmall, new CANNON.Vec3(mainAreaX + 
        narrowStripWidth, -floorThickness, 0));
    ...
    createHoles(){
        const corner = {x: Table.LENGTH/2 + 0.015, z: Table.WIDTH/2 + 0.015,
        PIby4: Math.PI/4};
    ...
    createWalls(){
        ...
        wall2.body.quaternion.setFromAxisAngle(new CANNON.Vec3(0,0,1),
        Math.PI);
        const wall3 = new LongWall(pos.x, pos.y, pos.z, 0.61);
        wall3.body.quaternion.setFromAxisAngle(new CANNON.Vec3(1,0,0),
        Math.PI);
        const wall4 = new LongWall(-pos.x, pos.y, pos.z, 0.61);
        wall4.body.quaternion.setFromAxisAngle(new CANNON.Vec3(0,1,0),
        -Math.PI);

        pos.x = Table.LENGTH/2
        const wall5 = new ShortWall(pos.x, pos.y, 0, 0.605);
        const wall6 = new ShortWall(-pos.x, pos.y, 0, 0.605);
        wall6.body.quaternion.setFromAxisAngle(new CANNON.Vec3(0,1,0),
        -1.5 * Math.PI);
        ...
        walls.forEach(wall => {
            ...
            if(debug){
                addCannonVisual(wall.body, 0xDD00, false, false);
            }
        })
    }
}
