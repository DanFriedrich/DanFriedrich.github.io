...
class Controller{
    ...
    update(dt=0.0167){   
        ...
        if(this.move.up!=0){
            const forward = this.forward.clone().applyQuaternion(this.target.
            quaternion);
            speed = this.move.up>0 ? this.speed * dt : this.speed * dt * 0.3;
            speed *= this.move.up;
            const pos = this.target.position.clone().add(forward.
        ...
        if(playerMoved){
            ...
            if(speed>0.03){
                if(this.overRunSpeedTime){
                    const elapsedTime = this.clock.elapsedTime - this.
                    overRunSpeedTime;
                    run = elapsedTime>0.5;
                }else{
        ...
        if(this.look.up==0 && this.look.right==0){
            ...
            const cameraXAxis = this.xAxis.clone().applyQuaternion(this.
            camera.quaternion);
