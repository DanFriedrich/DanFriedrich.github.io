...
class NPC{
	...
	withinAggroRange(){
		const distSq = this.object.position.distanceToSquared(this.enemy.
		position);
		return distSq<400;
	}
	...
	withinFOV( fov ){
		...
		const v1 = this.forward.clone().applyQuaternion(this.object.
		quaternion);
		const v2 = this.enemy.position.clone().sub(this.object.position).
		normalize();
	...
	seeEnemy(){
		const enemyVec = this.enemy.position.clone().sub(this.object.
		position);
		...
		const intersects = this.raycaster.intersectObjects(this.factory.
		children);
	...
	update(dt){
		...
		if(!this.dead && this.game.started && this.enemy && !this.enemy.
		userData.dead){
			if(!this.aggro){
				if(this.withinAggroRange()){
					if(this.withinFOV(120)){
						this.aggro = true;
						const v = this.enemy.position.clone().sub(this.
						object.position);
						const len = v.length();
						if(len>10){
				...
				if(this.withinAggroRange()){
					const v = this.enemy.position.clone().sub(this.object.
					position);
					const len = v.length();
					if(!this.isFiring){
						if(len<10){
