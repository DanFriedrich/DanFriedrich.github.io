...
class Game{
		...
		const tintShader = {
			...
			fragmentShader: /* glsl */`
				...
				void main() {
					...
					vec3 tint = tintColor * luminance * 1.875;
	...
	tintScreen(action){
		this.tintPass.uniforms.strength.value = 1.0;
		const duration = (action=='shot') ? 3.0 : 1.5;
		this.tween = new Tween(this.tintPass.uniforms.strength, 'value', 0,
		duration, this.removeTween.bind(this));
	}
